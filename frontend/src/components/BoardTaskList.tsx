...
import { BOARD_GT, SANS, MONO, T, PRI_COLOR, STATUS_CFG, STATUSES, PRIORITIES } from "../constants";
...
export default function BoardTaskList({ tasks, projectId, onUpdate, onDelete, onAdd, team }: BoardTaskListProps) {
  ...
  return (
    <div>
      {tasks.map(t => {
        ...
        if (confirmId === t.id) return (
          <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 14px 9px 10px", background: "rgba(212,114,114,0.05)", borderBottom: `1px solid ${T.borderSub}` }}>
            ...
            <button onClick={() => { onDelete(pid, t.id); setConfirmId(null); setExpandedId(null); }} style={{ background: "rgba(212,114,114,0.15)", border: "1px solid rgba(212,114,114,0.4)", color: "#d47272", cursor: "pointer", fontSize: 9, padding: "3px 12px", borderRadius: 6, fontFamily: SANS }}>DELETE</button>
          ...
          <div key={t.id} style={{ borderBottom: `1px solid ${T.borderSub}`, background: open ? `${T.accent}06` : "transparent" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: BOARD_GT, alignItems: "center", padding: "0 14px 0 10px", minHeight: 38 }}
              ...
              <div style={{ padding: "9px 8px 9px 0" }}>
                <span style={{ padding: "2px 7px", borderRadius: 6, fontSize: 9, fontFamily: MONO, fontWeight: 500, background: `${pc}22`, color: pc, border: `1px solid ${pc}40`, whiteSpace: "nowrap" }}>{cat}</span>
              </div>
              ...
              <div style={{ position: "relative", padding: "6px 8px 6px 0" }} onClick={e => e.stopPropagation()}>
                <div title="Click to change" onClick={() => setEditing({ taskId: t.id, field: "assignee" })}
                  style={{ fontSize: 11, fontFamily: SANS, color: m ? m.color : T.textSec, opacity: m ? 1 : 0.5, cursor: "pointer", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", borderRadius: 6, padding: "2px 4px", margin: "-2px -4px" }}
                  onMouseEnter={e => e.currentTarget.style.background = T.hover}
                    ...
                    renderOption={(aid: string | null) => {
                      ...
                      return (
                        <div key={aid || "none"} onClick={() => { upd("assignee", aid); stopEdit(); }}
                          style={{ padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontFamily: SANS, color: mb ? mb.color : T.textSec, background: t.assignee === aid ? T.hover : "transparent" }}
                          onMouseEnter={e => e.currentTarget.style.background = T.hover}
                          onMouseLeave={e => e.currentTarget.style.background = t.assignee === aid ? T.hover : "transparent"}>
                          {mb ? mb.name : "\u2014 none"}
                        </div>
                      );
                    }}
              ...
              <div style={{ padding: "12px 14px 14px 40px", borderTop: `1px solid ${T.border}`, background: `${T.accent}04` }}>
                ...
                <div style={{ marginTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <button onClick={() => setConfirmId(t.id)}
                    style={{ background: "none", border: "1px solid rgba(212,114,114,0.2)", color: "rgba(212,114,114,0.45)", cursor: "pointer", fontSize: 9, padding: "3px 12px", borderRadius: 6, fontFamily: SANS, letterSpacing: 1 }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(212,114,114,0.5)"; e.currentTarget.style.color = "#d47272"; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(212,114,114,0.2)"; e.currentTarget.style.color = "rgba(212,114,114,0.45)"; }}>DELETE TASK</button>
                </div>
              </div>
            )}
          </div>
      ...
      <button onClick={() => onAdd(projectId)} style={{ margin: "4px 10px 6px", padding: "4px 12px", background: `${T.accent}08`, border: `1px dashed ${T.accent}22`, color: `${T.accent}88`, cursor: "pointer", fontSize: 9, borderRadius: 6, fontFamily: SANS, letterSpacing: 1, display: "block" }}>+ ADD TASK</button>
    </div>
  );
}
