...
import { DISCIPLINES, DISC_COLOR, PRI_COLOR, STATUS_CFG, STATUSES, BOARD_GT, SANS } from "../constants";
...
export default function BoardView({ clients, domains, team, getT, updateTask, deleteTask, addTask, bGroup, setBGroup, bSort, setBSort, bFiltP, setBFiltP, bFiltS, setBFiltS, bFiltA, setBFiltA, bFiltD, setBFiltD, bSearch, setBSearch, threatColor }: BoardViewProps) {
  ...
  if (bGroup === "project") {
    ...
    groups = aIds.map(aid => { const m = team.find(x => x.id === aid); return { key: aid, label: m ? m.name : "Unassigned", color: m ? m.color : "#6a9aaa", sub: null, tasks: allTasks.filter(t => (t.assignee || "__none__") === aid) }; });
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* TOOLBAR */}
      <div style={{ flexShrink: 0, borderBottom: "1px solid rgba(123,104,238,0.09)", padding: "9px 18px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", background: "rgba(2,9,22,0.6)" }}>
        {/* SEARCH */}
        <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
          <span style={{ position: "absolute", left: 8, fontSize: 11, color: "rgba(255,255,255,0.25)", pointerEvents: "none" }}>{"\u2315"}</span>
          <input value={bSearch} onChange={e => setBSearch(e.target.value)} placeholder="Search tasks\u2026"
            style={{ paddingLeft: 24, paddingRight: bSearch ? 24 : 8, paddingTop: 4, paddingBottom: 4, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 4, color: "rgba(228,228,231,0.85)", fontSize: 11, fontFamily: SANS, outline: "none", width: 180, transition: "border 0.15s" }}
            onFocus={e => e.target.style.borderColor = "rgba(123,104,238,0.5)"}
            onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.09)"} />
          {bSearch && <span onClick={() => setBSearch("")} style={{ position: "absolute", right: 7, fontSize: 12, color: "rgba(255,255,255,0.3)", cursor: "pointer", lineHeight: 1 }}>{"\u2715"}</span>}
        </div>
        <div style={{ width: 1, height: 18, background: "rgba(123,104,238,0.12)" }} />
        {/* GROUP BY */}
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ fontSize: 9, letterSpacing: 1, color: "#4a7a9a" }}>GROUP</span>
          {([["project", "PROJECT"], ["priority", "PRIORITY"], ["status", "STATUS"], ["assignee", "ASSIGNEE"], ["discipline", "DISCIPLINE"]] as const).map(([v, l]) => (
            <button key={v} onClick={() => setBGroup(v)} style={{ padding: "3px 9px", fontSize: 9, letterSpacing: 0, fontFamily: "'Inter',sans-serif", cursor: "pointer", borderRadius: 3, background: bGroup === v ? "rgba(123,104,238,0.15)" : "transparent", border: `1px solid ${bGroup === v ? "rgba(123,104,238,0.4)" : "rgba(255,255,255,0.07)"}`, color: bGroup === v ? "#7B68EE" : "#6a9aaa" }}>{l}</button>
          ))}
        </div>
        <div style={{ width: 1, height: 18, background: "rgba(123,104,238,0.12)" }} />
        {/* SORT BY */}
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ fontSize: 9, letterSpacing: 1, color: "#4a7a9a" }}>SORT</span>
          {([["priority", "PRIORITY"], ["status", "STATUS"], ["task", "A\u2013Z"], ["assignee", "ASSIGNEE"]] as const).map(([v, l]) => (
            <button key={v} onClick={() => setBSort(v)} style={{ padding: "3px 9px", fontSize: 9, letterSpacing: 0, fontFamily: "'Inter',sans-serif", cursor: "pointer", borderRadius: 3, background: bSort === v ? "rgba(123,104,238,0.15)" : "transparent", border: `1px solid ${bSort === v ? "rgba(123,104,238,0.4)" : "rgba(255,255,255,0.07)"}`, color: bSort === v ? "#7B68EE" : "#6a9aaa" }}>{l}</button>
          ))}
        </div>
        <div style={{ width: 1, height: 18, background: "rgba(123,104,238,0.12)" }} />
        {/* FILTER: priority */}
        {PRIORITIES_LIST.map(p => (
          <button key={p} onClick={() => toggleFilter(bFiltP, setBFiltP, p)} style={{ padding: "3px 9px", fontSize: 9, letterSpacing: 0, fontFamily: "'Inter',sans-serif", cursor: "pointer", borderRadius: 3, background: bFiltP.includes(p) ? `${PRI_COLOR[p]}20` : "transparent", border: `1px solid ${bFiltP.includes(p) ? PRI_COLOR[p] + "55" : "rgba(255,255,255,0.07)"}`, color: bFiltP.includes(p) ? PRI_COLOR[p] : "#6a9aaa" }}>{p}</button>
        ))}
        <div style={{ width: 1, height: 18, background: "rgba(123,104,238,0.12)" }} />
        {/* FILTER: status */}
        {STATUSES.map(s => (
          <button key={s} onClick={() => toggleFilter(bFiltS, setBFiltS, s)} style={{ padding: "3px 9px", fontSize: 9, letterSpacing: 0, fontFamily: "'Inter',sans-serif", cursor: "pointer", borderRadius: 3, background: bFiltS.includes(s) ? `${STATUS_CFG[s]?.color}20` : "transparent", border: `1px solid ${bFiltS.includes(s) ? STATUS_CFG[s]?.color + "55" : "rgba(255,255,255,0.07)"}`, color: bFiltS.includes(s) ? STATUS_CFG[s]?.color : "#6a9aaa" }}>{STATUS_CFG[s]?.label || s}</button>
        ))}
        {activeFilters > 0 && <button onClick={() => { setBFiltP([]); setBFiltS([]); setBFiltA([]); setBFiltD([]); setBSearch(""); }} style={{ padding: "3px 9px", fontSize: 8, fontFamily: "inherit", cursor: "pointer", borderRadius: 3, background: "rgba(255,100,100,0.1)", border: "1px solid rgba(255,100,100,0.3)", color: "#ff8888", letterSpacing: 1 }}>{"\u2715"} CLEAR {activeFilters}</button>}
      </div>

      {/* DISCIPLINE + ASSIGNEE FILTER ROW */}
      <div style={{ flexShrink: 0, borderBottom: "1px solid rgba(123,104,238,0.07)", padding: "6px 18px", display: "flex", alignItems: "center", gap: 8, background: "rgba(2,9,22,0.4)" }}>
        <span style={{ fontSize: 9, letterSpacing: 1, color: "#4a7a9a" }}>FILTER</span>
        {DISCIPLINES.map(d => (
          <button key={d} onClick={() => toggleFilter(bFiltD, setBFiltD, d)} style={{ padding: "2px 8px", fontSize: 8, fontFamily: "inherit", cursor: "pointer", borderRadius: 3, background: bFiltD.includes(d) ? `${DISC_COLOR[d]}20` : "transparent", border: `1px solid ${bFiltD.includes(d) ? DISC_COLOR[d] + "55" : "rgba(255,255,255,0.07)"}`, color: bFiltD.includes(d) ? DISC_COLOR[d] : "#6a9aaa" }}>{d}</button>
        ))}
        <div style={{ width: 1, height: 16, background: "rgba(123,104,238,0.09)" }} />
        {team.map(m => (
          <button key={m.id} onClick={() => toggleFilter(bFiltA, setBFiltA, m.id)} style={{ padding: "2px 8px", fontSize: 8, fontFamily: "inherit", cursor: "pointer", borderRadius: 3, background: bFiltA.includes(m.id) ? `${m.color}20` : "transparent", border: `1px solid ${bFiltA.includes(m.id) ? m.color + "55" : "rgba(255,255,255,0.07)"}`, color: bFiltA.includes(m.id) ? m.color : "#6a9aaa" }}>{m.name}</button>
        ))}
        <button onClick={() => toggleFilter(bFiltA, setBFiltA, "__none__")} style={{ padding: "2px 8px", fontSize: 8, fontFamily: "inherit", cursor: "pointer", borderRadius: 3, background: bFiltA.includes("__none__") ? "rgba(255,255,255,0.1)" : "transparent", border: `1px solid ${bFiltA.includes("__none__") ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.07)"}`, color: bFiltA.includes("__none__") ? "#c8d8e8" : "#6a9aaa" }}>Unassigned</button>
        <div style={{ marginLeft: "auto", fontSize: 9, color: "#3a5a6a" }}>{allTasks.length} tasks</div>
      </div>

      {/* TABLE */}
      <div style={{ flex: 1, overflowY: "auto", padding: "0 14px 14px" }}>
        {/* Sticky column header */}
        <div style={{ position: "sticky", top: 0, zIndex: 10, background: "#0d1117", borderBottom: "1px solid #1e2130", marginBottom: 0 }}>
          <div style={{ display: "grid", gridTemplateColumns: BOARD_GT, gap: 0, padding: "6px 14px 6px 10px" }}>
            {["", "CAT", "PROJ / ITEM", "TASK / CONTEXT", "ASSIGNEE", "STATUS", "PRIORITY", "DUE", "\u229E"].map((h, i) => (
              <div key={i} style={{ fontSize: 8, letterSpacing: 1, color: "rgba(255,255,255,0.25)", fontFamily: "'JetBrains Mono',monospace", fontWeight: 400 }}>{h}</div>
            ))}
          </div>
        </div>
        {groups.map(g => (
          <div key={g.key} style={{ marginBottom: 16 }}>
            {bGroup !== "project" && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, marginBottom: 0, padding: "4px 10px", background: "rgba(255,255,255,0.02)", borderLeft: `2px solid ${g.color}`, borderBottom: "1px solid #1e2130" }}>
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, fontWeight: 700, color: g.color, flex: 1 }}>{g.label.toUpperCase()}</div>
                {g.sub && <div style={{ fontSize: 9, color: "#5a7a9a", fontFamily: "'Inter',sans-serif" }}>{g.sub}</div>}
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.25)", fontFamily: "'Inter',sans-serif", background: "rgba(255,255,255,0.05)", padding: "1px 7px", borderRadius: 10 }}>{g.tasks.length}</div>
              </div>
            )}
            {g.tasks.length === 0
              ? <div style={{ fontSize: 9, color: "#3a5a6a", paddingLeft: 10, paddingTop: 6, fontFamily: "'Inter',sans-serif" }}>No tasks</div>
